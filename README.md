> # ⚠️ 運用終了（2026-08-29）
>
> **このリポジトリの自動処理はすべて停止しました。** GitHub Actions のワークフロー5本
> （月次テーマ株レポート／週次速報／決算アラート／ウォッチリスト監視／パフォーマンス更新）を削除済みです。
>
> **移管先**: 友人への共有は [Thesis-Tracker](https://github.com/watapon-euco/Thesis-Tracker)（非公開）の
> `guest-digest` ワークフローへ一本化しました。共有する内容は**月1回のスクリーニング結果（採用・ウォッチ・見送り）だけ**で、
> 週次の値動き調査・決算アラート・ウォッチリスト監視は行いません。
> **LINEのチャネルと配信先グループは従来のまま**（`LINE_CHANNEL_TOKEN` / `LINE_GROUP_ID` の値を移管先の
> `GUEST_LINE_CHANNEL_ACCESS_TOKEN` / `GUEST_LINE_GROUP_ID` に設定）＝友人から見た通知経路は変わりません。
>
> 過去に公開したレポート（`docs/`）と実装（`src/`）は記録として残しています。
> **API キーの請求を止めるため**、不要になった GitHub Secrets（`GEMINI_API_KEY` / `ANTHROPIC_API_KEY` / `CHAT_PROXY_URL`）は
> リポジトリ設定から削除してください。以下は運用当時のドキュメントです。
