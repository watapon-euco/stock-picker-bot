...
**ポートフォリオ・ガイド** (Phase F3)
- 推奨銘柄バスケットの構築を支援する一覧（`src/utils/portfolio.py`、すべて決定論的）
- **推奨配分%**: 確信度（テーマ内順位）× 逆ボラティリティで算出し合計100%に正規化（1銘柄上限30%）
- **相対バリュエーション**: テーマ内 PER 中央値比で割安／割高／中立を判定
- **データ品質バッジ**: Stooq 代替や財務指標欠落の銘柄を「低」と明示し誤判断を防止
- **集中リスク警告**: 同一銘柄が複数テーマに重複する場合を検出して通知
